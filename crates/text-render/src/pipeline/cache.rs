...
impl Cache {
    ...
    pub fn new(device: &wgpu::Device, width: u32, height: u32) -> Self {
        ...
        let upload_buffer =  device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("text_render::Cache upload buffer"),
    ...
    pub fn update(&mut self, device: &wgpu::Device, staging_belt: &mut wgpu::util::StagingBelt, encoder: &mut wgpu::CommandEncoder, offset: [u16; 2], size: [u16; 2], data: &[u8]) {
        ...
        let mut padded_data = staging_belt.write_buffer(encoder, &self.upload_buffer, 0, NonZeroU64::new(0).unwrap(), device);

        for row in 0..height {
            padded_data[row * padded_width..row * padded_width + width].copy_from_slice(&data[row*width..(row + 1) * width])
        }
